...
void* pindahd(void *arg) {
	...
	path = (char *) arg; //arg disimpen ke variabel path
	
	strcpy(a, arg); //copy ke a
	strcpy(b, arg); //copy ke b
	char *split1, *split2, *titik[5], *slash[5]; 
	int n = 0;
	split1 = strtok(path, "."); //untuk mengambil format file tanpa titik
	while(split1 != NULL) { //untuk ngecek ngambil format setiap file
		titik[n] = split1;
		n++;
		split1 = strtok(NULL, ".");
	}

	if(n != 1) { //untuk supaya format extensi kapital dan kecil bisa dianggap sama
		int a;
...
int main(int argc, char *argv[]) {
	...
	if(strcmp(argv[1], "-f") == 0) {
		...
		if (dr == NULL) {
			printf("Direktori tidak ada isinya" );
		}
		else {
			i = 0;
